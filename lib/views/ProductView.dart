...
class _ProductView extends State<ProductView> {
  ...
  void initState() {
    Order o;
    if (_cartBloc.currentCart.orders.isNotEmpty) {

      try {
        o = _cartBloc.currentCart.orders.firstWhere((e) =>
        e?.product?.id ==
            widget.product.id);
      }
      catch (e) {
    ...
    return  Scaffold(backgroundColor: Colors.white, appBar:  AppBar(backgroundColor: Colors.white, elevation: 0, iconTheme: IconThemeData(color: Colors.black)), body:
         SafeArea(child:
           Flex(
               ...
               children: <Widget>[
             Expanded(flex:4,
               child: Container(padding: EdgeInsets.symmetric(horizontal: 20), height: MediaQuery.of(context).size.height*0.73, child:
                 SingleChildScrollView(child:
                   Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                     Center(child:
                       StreamBuilder(initialData: null, stream: _cartBloc.observableLastOrder, builder: (context, AsyncSnapshot<Order> snapshot){
                        String tag = snapshot.data == null ? "tagHero${widget.product.id}" : "tagHeroOrder${snapshot.data.id}";
                        return  Hero(tag: tag, child:
                           Image.asset(widget.product.urlToImage, fit: BoxFit.cover, height: MediaQuery.of(context).size.height*0.4)
                        );
                      })
                    ),
                     Container(margin: EdgeInsets.only(top: 20), child:
                       Text(widget.product.title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 40, color: Colors.black)),
                    ),
                     Container(margin: EdgeInsets.only(top: 10), child:
                       Text("${widget.product.weight}g", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.grey)),
                    ),
                     Container(margin: EdgeInsets.only(top: 20), child:
                         Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
                           ...
                           Text("$currencyString${(widget.product.price*_quantity).toStringAsFixed(2)}", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 35, color: Colors.black)),
                        ])
                    ),
                     Container(margin: EdgeInsets.only(top: 40, bottom: 40), child:
                       Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                         Text("About the product:", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 20)),
                         Padding(padding: EdgeInsets.only(top: 10), child:  Text(widget.product.about, style: TextStyle(color: Colors.grey, fontSize: 18)))
                      ])
                    )
                  ])
                )
                ...
                BoxShadow(
                 ...
                 Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
                   ...
                   SizedBox(width: MediaQuery.of(context).size.width * 0.6, child:
                     RaisedButton(color: Colors.amber, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(60)), padding: EdgeInsets.all(20),
                        onPressed: (){
                        ...
                        child:  Text(
                          _quantity>startingQuantity?
                            "Add to cart"
                            :
                          (_quantity==0 && startingQuantity>0)?
                              "Remove Item":
                            "Back",
